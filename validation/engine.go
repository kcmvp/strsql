...
func (e *Engine) BeforeInsert(entity strsql.Entity) error {
	return e.Validate(entity, EventSave)
}
...
func (e *Engine) Validate(entity strsql.Entity, event Event, fields ...string) error {
	...
	if !partial {
		if ers, ok := e.registry.entRules[typ]; ok {
			for _, er := range ers {
				...
				res := fnVal.Call([]reflect.Value{reflect.ValueOf(entity)})
